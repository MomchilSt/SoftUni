...
namespace CosmosX.Core
{
    public class ReactorManager : IManager
    {
        ...
        public string ModuleCommand(IList<string> arguments)
        {
            ...
            switch (moduleType)
            {
                case "CryogenRod":
                    ...
                    break;
                case "HeatProcessor":
                    IAbsorbingModule absorbingModule = new HeatProcessor(this.currentId, additionalParameter);
                    this.reactors[reactorId].AddAbsorbingModule(absorbingModule);
                    this.identifiableObjects.Add(absorbingModule.Id, absorbingModule);
                    this.modules.Add(absorbingModule.Id, absorbingModule);
                    break;
                case "CooldownSystem":
        ...
        {
            long cryoReactorCount = this.identifiableObjects
                .Values
                .Count(r => r.GetType().Name == nameof(CryoReactor));

            long heatReactorCount = this.identifiableObjects
                .Values
